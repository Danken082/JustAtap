...
<head>
    ...
    <title>Edit Product | Just A Tap</title>
    ...
    <style>
        ...
        * { box-sizing: border-box; }
        ...
        .thumb-item input { width: auto; margin: 0; }
...
<body>
    <main class="wrap">
        ...
        <form method="POST" action="{{ route('admin.products.update', $product) }}" enctype="multipart/form-data">
            ...
            <section class="panel">
                ...
                <div class="field">
                    ...
                    @if ($product->mainImageUrl())
                        <img class="current-main" src="{{ $product->mainImageUrl() }}" alt="{{ $product->name }}">
                    @else
                        <p class="muted" style="color:var(--muted);">No main image set.</p>
                    @endif
                    <label for="main_image">Replace Main Image</label>
            ...
            <section class="panel">
                ...
                @foreach ($product->sizes as $size)
                    <div class="repeat-item">
                        <div class="grow">
                            <input type="text" name="existing_sizes[{{ $size->id }}]" value="{{ $size->name }}">
                        </div>
                        <label style="display:flex;align-items:center;gap:6px;color:#ffd7d7;font-size:0.8rem;">
                            <input type="checkbox" name="delete_size_ids[]" value="{{ $size->id }}"> Delete
                        </label>
                    </div>
                @endforeach
            ...
            <section class="panel">
                ...
                <p style="margin-top:0;color:var(--muted);">Each color can have multiple images. Add new images or remove existing ones.</p>
                    ...
                    <div class="repeat-item">
                        <div class="grow">
                            <div class="field">
                                ...
                                <input type="text" name="existing_colors[{{ $color->id }}][name]" value="{{ $color->name }}">
                            </div>
                                ...
                                <div class="thumb-grid">
                                    @foreach ($color->images as $image)
                                        <div class="thumb-item">
                                            <img src="{{ $image->url() }}" alt="{{ $color->name }}">
                                            <label>
                                                <input type="checkbox" name="delete_image_ids[]" value="{{ $image->id }}"> Delete
                                            </label>
                                        </div>
                                    @endforeach
                                </div>
                            ...
                            <div class="field" style="margin-bottom:0;">
                                ...
                                <input type="file" name="existing_colors[{{ $color->id }}][images][]" accept="image/*" multiple>
                            </div>
                        </div>
                        <label style="display:flex;align-items:center;gap:6px;color:#ffd7d7;font-size:0.8rem;">
                            <input type="checkbox" name="delete_color_ids[]" value="{{ $color->id }}"> Delete color
                        </label>
                    </div>
                @endforeach
    ...
    <template id="size_template">
        <div class="repeat-item">
            <div class="grow">
                <input type="text" name="new_sizes[]" placeholder="e.g. Standard">
            </div>
    ...
    <template id="color_template">
        <div class="repeat-item">
            <div class="grow">
                <div class="field">
                    <label>Color Name</label>
                    <input type="text" name="new_colors[__INDEX__][name]" placeholder="e.g. Midnight Black">
                </div>
                <div class="field" style="margin-bottom:0;">
                    <label>Images</label>
                    <input type="file" name="new_colors[__INDEX__][images][]" accept="image/*" multiple>
                </div>
            </div>
            <button type="button" class="icon-btn" data-remove-item>Remove</button>
        </div>
