    ...
    <main class="wrap">
        ...
        <section class="panel">
            ...
            <div class="preview-card" id="profilePreview" style="background: {{ $profile?->background_color ?? '#111827' }}; color: {{ $profile?->text_color ?? '#f9fafb' }}; border-color: {{ $profile?->accent_color ?? '#60a5fa' }};">
                <div>
                    <div class="title" id="previewDisplayName">{{ $profile?->display_name ?? ($selectedUser?->name ?? 'Display name') }}</div>
                ...
                <tbody>
                    @forelse($cards as $card)
                        <tr>
                            ...
                            <td>{{ $card->name }}</td>
                            <td>{{ $card->created_at ? 'Assigned on generation' : '' }}</td>
                            <td>{{ $card->created_at?->format('M d, Y h:i A') }}</td>
                        </tr>
                    @empty
                        <tr><td colspan="5" class="helper">No cards found.</td></tr>
                    @endforelse
                </tbody>
            </table>
        </section>
